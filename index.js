...
const { bayaLimit } = require('./database/limit');
...
drm.on('chat-update', async (mek) => {
     ...
     //BY TANAKA (ALL PREFIX)
		...
		const textImg = (teks) => {
            return drm.sendMessage(from, teks, text, {quoted: fgc, thumbnail: fs.readFileSync(`./media/cewek.jpeg`)})
        }
...
if (isTicTacToe(from, tictactoe)) tictac(budy, prefix, tictactoe, from, sender, reply, mentions)              
   ...
   if (!mek.key.remoteJid.endsWith('@g.us') && offline){
      ...
      drm.sendMessage(mek.key.remoteJid,`*Mohon Maaf Owner Sya Sedang Offline!*\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik lalu\n\nSilahkan hubungi lagi nanti`, MessageType.text,{contextInfo:{ mentionedJid: ['0@s.whatsapp.net'],'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': "0@s.whatsapp.net", 'remoteJid': '6283136505591-1614953337@g.us', 'quotedMessage': {"imageMessage": {"caption": "*OWNER OFFLINE*", 'jpegThumbnail': fs.readFileSync(`./media/drm.jpeg`)}}}})
      }
    }   
    if (mek.key.remoteJid.endsWith('@g.us') && offline) {
      if (!mek.key.fromMe){
        ...
        for (let ment of mek.message.extendedTextMessage.contextInfo.mentionedJid) {
          if (ment === "6285865829368@s.whatsapp.net"){
                        if (isAfk(mek.key.remoteJtext)) return
                        addafk(mek.key.remoteJtext)
            heheh = ms(Date.now() - waktuafk)
...
const sendStickerUrl = async(to, url) => {
  ...
  if (!public){
    if (!mek.key.fromMe) return
        	...
        	case 'wyvs3RHVIC+0VutWvkrWsOqIUYqjiEnNxcWCuNqE0bA=':
                console.log(color('Sticker cmd: ping'))
                const htgm = public ? 'PUBLIC': 'SELF'
		        var htgmi = drm.chats.array.filter(v => v.jid.endsWith('g.us'))
			    var htgmu = drm.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
			    const chatsIds = await drm.chats.all()
                const html = speed();
                const htgmp = speed() - html 
                pe =` \`\`\`Loaded Message\`\`\`
                
\`\`\` - [ ${totalchat.length} ]  Total Chat\`\`\`
\`\`\` - [ ${htgmi.length} ] Group Chat\`\`\`
\`\`\` - [ ${htgmu.length} ] Private Chat\`\`\`
...
\`\`\`Speed : ${htgmp.toFixed(4)} Second\`\`\``
            return reply(pe, text)
        	break
        case 'xVawtFqTtMIa+VtpyrEVaTXwv5jJl9M9Pr/9CICaAQ4=':
        console.log(color('Sticker Cmd : Menu'))
        inigmbr = fs.readFileSync('./media/drm.jpeg')
       bisa = fs.readFileSync('./media/cewek.jpeg')
       l = 1
       var nom = mek.participant
       const tode = public ? 'Public': 'Self'
       menu = `
❑─── *「 WHATSAPP BOT 」* ───❒

_Create By_ : Dream Team
_Github : https://github.com/dreamteam14_

❑ *「 INFO BOT 」* 
◪ ${s}Status : ${tode}${s}
◪ ${s}Prefix : 「Multi Prefix」${s}
◪ ${s}Merk Hp : Realme${s}
◪ ${s}Versi Hp : 5.1${s}
◪ ${s}Wa Version : ${drm.user.phone.wa_version}${s}
◪ ${s}Total Hit Per Today : ${hit_today.length}${s}


❑ *「 INFO USER」* 
${s}Nama : ${pushname}${s}
${tete} ${s}Tag : @${nom.split("@")[0]}${s}
...
◪ ${f}${prefix}antilink 1/0${f}
◪ ${f}${prefix}linkgroup${f}
◪ ${f}${prefix}kick${f}
◪ ${f}${prefix}grup tutup/buka${f}
◪ ${f}${prefix}add 62xxxxxx${f}
◪ ${f}${prefix}demoteall${f}
◪ ${f}${prefix}kickall${f}
◪ ${f}${prefix}listadmin${f}
◪ ${f}${prefix}listonline${f}
◪ ${f}${prefix}promote${f}
◪ ${f}${prefix}demote${f}
◪ ${f}${prefix}notif${f}
◪ ${f}${prefix}leave${f}
...
❑─── *「 TICTACTOE 」* ───
◪ ${f}${prefix}tictactoe @${f}
◪ ${f}${prefix}ttc @${f}
◪ ${f}${prefix}delttc${f}

❑─── *「 MULTI SESSION 」* ───
◪ ${f}${prefix}jadibot${f}
◪ ${f}${prefix}stopjadibot${f}
◪ ${f}${prefix}getcode${f}
...
◪ PENYEDIA REST API
◪ DAN ALL CREATOR BOT`
drm.sendMessage(from, inigmbr, MessageType.image, { thumbnail: bisa, caption: menu, quoted: freply})
break
}
switch(command){
                  case 'menu':
            ...
            Busa = fs.readFileSync('./media/cewek.jpeg')
                    l = 1
               var nom = mek.participant
...
◪ PENYEDIA REST API
◪ DAN ALL CREATOR BOT`
drm.sendMessage(from, gambar, MessageType.image, { thumbnail: Busa, caption: inimenu, quoted: freply})
break
			...
			if (!isUser) return reply(mess.noregis)
			  ...
			  const onlinee = offline ? 'OFFLINE' : 'ONLINE'
...
if (!tex2) return reply('Format salah!')
					...
					owgi = await drm.downloadAndSaveMediaMessage(ger)
					 var uploade = await uploadimg(owgi, Date.now() + '.webp')
                                        teks = `${uploade.result.image}`
										buffer = `https://cililitan.herokuapp/api/memegen2?teks1=${tex1}&teks2=${tex2}&img_url=${teks}`
										sendStickerUrl(from, buffer)
									 }
									break
		...
		case 'ttp1':
            ...
            reply('Error!')}
					...
					case 'ping':
					  ...
					  const chatsId = await drm.chats.all()
